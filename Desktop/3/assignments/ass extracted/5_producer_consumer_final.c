...
void producer(int *buf, int *index, int *remain)		//Producer function
{
...
void consumer(int *buf, int *sum, int *index, int *remain)		//Consumer function
{
			...
			if( flag == 't' )
			{
				*forked = *forked + 1;
				if( *forked <= prod )
				{
					...
					producer(buf, forked, remaining_pro);
					printf("\nExiting producer\n");
					shmdt(buf);
					shmdt(sum);
					shmdt(forked);
				...
				else
				{
					...
					consumer(buf, sum, forked, remaining_pro);
					printf("\nExiting consumer\n");
					shmdt(buf);
					shmdt(sum);
					shmdt(forked);
